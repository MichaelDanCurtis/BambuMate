...
must_haves:
  ...
  artifacts:
    ...
    - path: "src-tauri/src/commands/analyzer.rs"
      provides: "apply_recommendations command"
...
Output: apply_recommendations command, history commands (list, get, revert), analyze_print modified to record sessions automatically.
</objective>
...
<task type="auto">
  <name>Task 3: Register commands and update analyze_print</name>
  <files>
    src-tauri/src/lib.rs
    src-tauri/src/commands/analyzer.rs
  </files>
...
**In analyzer.rs**, modify analyze_print to record sessions automatically:

After the analysis succeeds but before returning, add:
```rust
...
if let Ok(history) = RefinementHistory::new(&db_path) {
    ...
    let analysis_json = serde_json::to_string(&AnalyzeResponse {
        defect_report: defect_report.clone(),
        recommendations: recommendations.clone(),
        conflicts: evaluation.conflicts.clone(),
    ...
    if let Err(e) = history.record_analysis(&profile_for_history, image_for_history, &analysis_json) {
...
Note: This adds optional session recording. Failures don't break analysis.
  </action>
  <verify>
`cargo check -p bambumate` passes.
...
analyze_print automatically records sessions to history.
  </done>
</task>
...
4. analyze_print records sessions (non-fatally on error)
</verification>
...
- analyze_print records session (failure doesn't break analysis)
</success_criteria>
